...
router.route("/")
  .get((req, res, next) => {
    ...
    let fetchedData = Organization.find(queryObject).select("-password").populate("org_projects", "title description uid thumbnail");
...
router.route("/:uid")
  .get((req, res, next) => {
    Organization.findOne({ uid: req.params.uid }).select("-password").populate("org_projects", "title description uid thumbnail")
      .then((document) => {
        if (!document) {
          throw Error("Organization not found.");
        }
